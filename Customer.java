...
public class Customer extends User implements UserAuthentication {
    ...
    String pinCode = "";
    while (pinCode.length() != 4 || !pinCode.matches("\\d+")) {
        System.out.print("Create a 4-digit PIN code for this account: ");
        pinCode = scanner.nextLine();
        if (pinCode.length() != 4) {
            System.out.println("PIN must be exactly 4 digits. Please try again.");
        } else if (!pinCode.matches("\\d+")) {
            System.out.println("PIN must contain only numbers. Please try again.");
        }
    }
    
    String confirmPin = "";
    while (!pinCode.equals(confirmPin)) {
    ...
    BankAccount account = new BankAccount(accountNumber, type, initialBalance, pinCode);  
    ...
    System.out.println("========================================");
    System.out.println("Please save your account number and PIN for future transactions.");
}
    ...
    if (!fromAccount.verifyPin(enteredPin)) {
        System.out.println("Incorrect PIN. Transaction cancelled.");
        return;
    }
    ...
    // ADD THESE TWO METHODS TO CUSTOMER CLASS

public static Customer register(Scanner scanner, ArrayList<Customer> existingCustomers, int idCounter) {
    ...
    while (username.trim().isEmpty() || usernameTaken) {
    ...
    String customerID = "CUST" + (existingCustomers.size() + 1);
...
public static Customer authenticate(Scanner scanner, ArrayList<Customer> customers) {
    System.out.println("\n--- Customer Login ---");
